# -*- coding: utf-8 -*-
"""
v2_adapter.py - Adapter for feature_table_v2

feature_table_v2 に対応するアダプター。
v2はhr_*特徴量を含むが、ax*_は含まない。
"""
...
@register_adapter("v2")
class V2Adapter(BaseFeatureAdapter):
    """
    feature_table_v2 用アダプター

    v2は hr_* 特徴量を含む。
    """

    VERSION_NAME = "v2"
    DESCRIPTION = "feature_table_v2 (with hr_* features)"
    ...
    def get_used_features(
        ...
        必ずversion付きファイル (feature_columns_{target}_v2.json) を参照する。
        """
        # 1. version付きfeature_columns JSONファイルを探す（fallback禁止）
        if self.models_dir:
            json_path = self.models_dir / f"feature_columns_{target}_v2.json"
            if json_path.exists():
                ...
                if fallback_path.exists():
                    logger.warning(
                        f"[v2] Found version-less file {fallback_path.name} but NOT using it. "
                        f"Please run train_eval_legacy.py --version v2 to generate v2-specific files."
                    )

        # 2. テーブルから特徴量を取得（モデルファイルがない場合のみ）
        logger.warning(f"[v2] No feature_columns_{target}_v2.json found, using table columns")
    ...
    def get_importance(
        ...
        if self.models_dir:
            csv_paths = [
                self.models_dir / f"feature_importance_{target}_v2.csv",
            ]
        ...
        if self.models_dir:
            model_paths = [
                self.models_dir / f"lgbm_{target}_v2.txt",
            ]
