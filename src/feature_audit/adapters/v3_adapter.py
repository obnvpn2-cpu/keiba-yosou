# -*- coding: utf-8 -*-
"""
v3_adapter.py - Adapter for feature_table_v3

feature_table_v3 に対応するアダプター。
v3はhr_*やax*_の拡張特徴量を含む最新のlegacyテーブル。
"""
...
@register_adapter("v3")
class V3Adapter(BaseFeatureAdapter):
    """
    feature_table_v3 用アダプター

    v3は hr_*, ax*_ 特徴量を含む。
    """

    VERSION_NAME = "v3"
    DESCRIPTION = "feature_table_v3 (with hr_*, ax*_ features)"
    ...
    def get_importance(
        ...
        if self.models_dir:
            csv_paths = [
                self.models_dir / f"feature_importance_{target}_v3.csv",
            ]
        ...
        if self.models_dir:
            model_paths = [
                self.models_dir / f"lgbm_{target}_v3.txt",
            ]
